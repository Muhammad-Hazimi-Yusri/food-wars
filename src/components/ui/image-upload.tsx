...
export function ImageUpload({
  ...
  const [lastPropValue, setLastPropValue] = useState<string | null>(previewUrl);

  // Sync with previewUrl prop when it changes (e.g., async load).
  // Setting state during render is the documented React 19 alternative
  // to setState-in-effect for prop-to-state sync.
  if (previewUrl !== lastPropValue) {
    setLastPropValue(previewUrl);
