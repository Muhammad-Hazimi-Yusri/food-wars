-- ============================================
-- MIGRATION 016: Unit System Improvements
-- - Add quick_consume_as_percentage to products
-- - Add 'pint' unit to all households
-- - Update handle_new_user() to seed 'pint'
-- - Update seed_guest_data() to include 'pint'
-- SI conversions (kg↔g, L↔mL, pint↔mL) are
--   now resolved in application code, not DB.
-- ============================================

-- 1. Add percentage mode flag to products
ALTER TABLE products
  ADD COLUMN quick_consume_as_percentage BOOLEAN NOT NULL DEFAULT false;

-- 2. Add 'pint' to all existing authenticated households
INSERT INTO quantity_units (household_id, name, name_plural, sort_order)
SELECT h.id, 'pint', 'pints', 11
FROM households h
WHERE h.id != 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'
  AND NOT EXISTS (
    SELECT 1 FROM quantity_units qu
    WHERE qu.household_id = h.id AND qu.name = 'pint'
  );

-- 3. Add 'pint' to guest household
INSERT INTO quantity_units (id, household_id, name, name_plural, sort_order) VALUES
  ('40000000-0000-0000-0000-000000000013', 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11', 'pint', 'pints', 13)
ON CONFLICT (id) DO NOTHING;

-- 4. Update handle_new_user() to seed 'pint' for new households
...
BEGIN
  ...
  INSERT INTO quantity_units (household_id, name, name_plural, sort_order) VALUES
    ...
    (new_household_id, 'pint', 'pints', 11);
...
-- 5. Update seed_guest_data() to include 'pint'
...
BEGIN
  ...
  INSERT INTO quantity_units (id, household_id, name, name_plural, sort_order) VALUES
    ...
    ('40000000-0000-0000-0000-000000000013', guest_hh_id, 'pint', 'pints', 13);
